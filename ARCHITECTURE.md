...
* **Interactive TUI:** Provides a low-latency, keyboard-driven alternative to the Web UI, showing that the engine can handle stateful navigation and real-time terminal rendering.
...
1. **Web UI (`cmd/grip-web`):** The primary dashboard.
2. **JSON API (`cmd/grip-api`):** Programmatic access with Swagger.
3. **CLI (`cmd/grip-cli`):** High-speed terminal output.
4. **TUI (`cmd/grip-tui`):** Interactive terminal dashboard.